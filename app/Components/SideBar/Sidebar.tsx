...
function Sidebar() {
  ...
  return (
    <div style={{color:isDarkMode?darkModeColor.textColor:defaultColor.textColor, backgroundColor:isDarkMode ? darkModeColor.background : defaultColor.background}} 
       ref={sideBarRef}
       className={` ${!openSideBar ? "max-xl:hidden" : "fixed shadow-lg"}  flex-grow z-50   p-10 flex flex-col gap-20 bg-white transition-all`}>
        <LogoAnName/>
        <MenuSelection/>
        <LogoutSection/>
    </div>
